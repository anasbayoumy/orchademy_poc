...
// Customizable cards component: three slots where user can choose any category
function CustomizableCards({ categories, colors }: { categories: any[]; colors: any }) {
    const [selections, setSelections] = useState<number[]>([2, 11, 12]);

    const handleChange = (slot: number, value: number) => {
        setSelections((prev) => {
            const copy = [...prev];
            copy[slot] = value;
            return copy;
    ...
    return (
        <div className="mb-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            {selections.map((sel, i) => {
                const cat = categories[sel] ?? categories[0];
                const catColor = cat.categoryType === 'revenue' ? colors.secondary1 : cat.categoryType === 'expense' ? colors.primary1 : colors.secondary3;

                return (
                    <div
                        key={i}
                        className="rounded-xl p-6 flex flex-col justify-between transform transition-transform hover:-translate-y-1 hover:shadow-lg"
                        style={{ backgroundColor: colors.cardBg, border: `2px solid ${catColor}`, minHeight: '170px' }}
                    >
                        <div>
                            <p className="text-lg font-bold mb-3 truncate" style={{ color: colors.textPrimary }}>{cat.title.replace(/Category \d+: /, '')}</p>

                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-2xl font-bold" style={{ color: colors.textPrimary }}>{formatCurrency(cat.value)}</p>
                                    <p className="text-xs" style={{ color: colors.textSecondary }}>2023-24 • CAGR: {calculateCAGR(cat.trendData.map((d: any) => d.value)).toFixed(1)}%</p>
                                </div>
                                <div className="text-right">
                                    <span className="text-sm px-2 py-1 rounded font-semibold inline-block"
                                        style={{
                                            backgroundColor: parseFloat(cat.growth) >= 0 ? colors.successBg : colors.dangerBg,
                                            color: parseFloat(cat.growth) >= 0 ? colors.successText : colors.dangerText
                                        }}
                                    >
                                        {parseFloat(cat.growth) >= 0 ? '+' : ''}{cat.growth}% YoY
                                    </span>
                                </div>
                            </div>
                        </div>

                        {/* footer with selector moved to bottom-right for easier access */}
                        <div className="mt-4">
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    style={{
                                        ...
                                        backgroundColor: colors.isDark ? '#0b1220' : '#ffffff',
                                        border: 'none',
                                        borderRadius: 12,
                                        padding: '0.6rem 2.5rem 0.6rem 0.9rem',
                                        fontSize: '0.95rem',
                                        boxShadow: colors.isDark ? '0 1px 2px rgba(2,6,23,0.6)' : '0 1px 2px rgba(2,6,23,0.04)'
                                    }}
                                >
                                    {categories.map((c, idx) => (
                                        <option key={idx} value={idx}>{c.title.replace(/Category \d+: /, '')}</option>
                                    ))}
                                </select>
...
export default function PLFunctionalPage() {
    ...
    return (
        <div className="animate-fade-in" style={{ direction: isRTL ? 'rtl' : 'ltr' }}>
            ...
            {/* Customizable summary cards: three slots selectable from categories */}
            {expandedCard === null && (
                <CustomizableCards categories={categories} colors={colors} />
            )}
